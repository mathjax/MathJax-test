...
def printReftestList():
    ...
    fp = file(ROOT + "web/reftestList.js", "w")
    stdout = sys.stdout
    sys.stdout = fp
...
def runTestingInstance(aDirectory, aSelenium, aSuite):
    ...
    # Build the testsuite
    aSuite.sendRequest("Initializing", "-")
                ...
                for browserMode in browserModeList2:
                    ...
                    if browserStartCommand != "unknown":
                        ...
                        if (clearBrowsersData):
                            ...
                            suite = reftest.reftestSuite(aTransmitToTaskHandler,
                                                         taskName[i],
                                                         runSlowTests,
                                                         runSkipTests,
                                                         listOfTests,
...
def announceDeath(aDeath):
    HOST, PORT = "localhost", 4445
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((HOST, PORT))
        sock.send("TASKDEATH " + aDeath + " " + str(os.getpid()))
    except socket.error:
        return
