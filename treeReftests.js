...
/**
 *  @file
 *  This file is for test pages of type @ref reftest::treeReftest, where we
 *  compare two DOM trees.
 *  There are two ways to define such a test. The first one is to consider a
 *  <math> element as the tree to serialize. To do that, you have to put such
 *  an element as a descendant of one of id "reftest-element". For instance,
 *  "<div id="reftest-element"><math>...</math></div>". In that case, the ouput
 *  is automatically set to nativeMML, so that you can also do things like
 *  "<div id="reftest-element">$$x+1$$</div>". The script looks inside the
